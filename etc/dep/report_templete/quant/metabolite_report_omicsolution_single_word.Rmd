...
cat("## 差异信息\n\n")
tmp2 <- final_summary[, -c(3:6), drop=FALSE]
...
cat('* > 打开缺失值过滤数据表: `./2.数据预处理/1.缺失值过滤/组内缺失值过滤数据_single.xlsx`\n\n')
cat('* > 打开缺失值填充数据表: `./2.数据预处理/2.缺失值填充/缺失值填充数据_single.xlsx`\n\n')
cat('* > 打开RSD过滤数据表: `./2.数据预处理/3.RSD过滤/RSD过滤数据_single.xlsx`\n\n')
cat('* > 打开归一化数据表: `./2.数据预处理/4.归一化/归一化处理数据_single.xlsx`\n\n')
...
include_img(
  png_path = file.path(report_path, "3.QC/1.TIC/TIC_single.png")
)
...
include_img(
  png_path = file.path(report_path, "3.QC/2.correlation/single/correlation_single.png"),
  pdf_path = file.path(report_path, "3.QC/2.correlation/single/correlation_single.pdf")
)
...
include_img(
  png_path = file.path(report_path, "3.QC/3.RSD/single/RSD_single.png")
)
...
include_img(
  png_path = file.path(report_path, "3.QC/4.PCA/single/PCA_single.png")
)
...
if (length(unique(sample.data[[ion_type]][["Condition"]][!grepl("qc", sample.data[[ion_type]][["Condition"]], ignore.case = TRUE)])) > 1) {
  ...
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/1.PCA", group.data[1], "single", sprintf("PCA_%s_single.png", group.data[1]))
  )
...
if (length(unique(sample.data[[ion_type]][["Condition"]][!grepl("qc", sample.data[[ion_type]][["Condition"]], ignore.case = TRUE)])) > 1) {
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/2.PLS-DA", group.data[1], "single", sprintf("PLS-DA_%s_single.png", group.data[1]))
  )
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/2.PLS-DA", group.data[1], "single", sprintf("PLS-DA_%s_single_permutation_plot.png", group.data[1]))
  )
...
if (length(unique(sample.data[[ion_type]][["Condition"]][!grepl("qc", sample.data[[ion_type]][["Condition"]], ignore.case = TRUE)])) > 1) {
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/3.OPLS-DA", group.data[1], "single", sprintf("OPLS-DA_%s_single.png", group.data[1]))
  )
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/3.OPLS-DA", group.data[1], "single", sprintf("OPLS-DA_%s_single_permutation_plot.png", group.data[1]))
  )
  include_img(
    png_path = file.path(report_path, "4.多元统计分析/3.OPLS-DA", group.data[1], "single", sprintf("S-Plot_%s_single.png", group.data[1]))
  )
}

cat("注：图中横坐标表示置换检验的置换保留度（与原模型Y变量顺序一致的比例，置换保留度等于1处的点即为原模型的R2Y和Q2值），纵坐标表示R2Y或Q2 的取值，绿色三角形表示置换检验得到的R2Y值，蓝色圆点表示置换检验得到的Q2值，两条虚线分别表示R2Y和Q2 的回归线。原模型R2Y非常接近1，说明建立的模型符合样本数据的真实情况；原模型Q2 非常接近1，说明如果有新样本加入模型，会得到近似的分布情况，总的来说原模型可以很好地解释两组样本之间的差异。置换检验随机模型的Q2值均小于原模型的Q2 值；Q2的回归线与纵轴的截距小于零；同时随着置换保留度逐渐降低，置换的Y变量比例增大，随机模型的Q2逐渐下降，且Q2截距值小于0，说明原模型具有良好的稳健性，不存在过拟合现象。\n\n")

cat("注：横坐标表示第一主成分解释度，纵坐标表示正交成分解释度，散点的颜色和形状表示样本的实验分组。椭圆代表95%置信区间。\n\n")
...
# 差异筛选（保持原有逻辑，仅展示第一组）
if (nrow(final_summary) > 0 &&
    sum(final_summary$diff_feature) > 0 &&
    sum(final_summary$diff_feature_name) > 0) {
  ...
  cat("## 分组差异结果表\n\n")
  cat(paste0("### ", group.data[1], "\n\n"))
  cat(paste0("* > 单离子表格：`./5.差异筛选/", group.data[1], "/", group.data[1], "_表格拆分与去重_single.xlsx`\n\n"))
...
if (nrow(final_summary) > 0 &&
    ...
    vn <- file.path(report_path, "6.差异代谢物可视化", group.data[1], "3.volcano", "volcano_name.xlsx")
    if (file.exists(vn)) {
...
对每一组对比，我们对差异代谢物进行聚类，并以热力图进行展示。\n\n")
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    pdf_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "1.HCA", "heatmap.pdf")
  )
  include_img(
    pdf_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "1.HCA", "heatmap_name.pdf")
  )
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    pdf_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "2.correlation", "correlation_top20_features.pdf")
  )
  include_img(
    pdf_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "2.correlation", "correlation_top20_features_name.pdf")
  )
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "3.volcano", "volcano.png")
  )
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "3.volcano", "volcano_name.png")
  )
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "4.VIP_analysis", "VIP_distribution.png")
  )
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "4.VIP_analysis", "VIP_bubble.png")
  )
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "4.VIP_analysis", "VIP_bubble_name.png")
  )
...
if (nrow(final_summary) > 0 &&
    ...
    # 点图
    include_img(
      png_path = file.path(report_path, "7.KEGG富集", group.data[1], "KEGG_dotplot.png")
    )
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
    include_img(
      pdf_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "5.差异代谢物表达量", "Global_Metabolite_Comparison.pdf")
    )
  ...
  cat(paste0("### ", group.data[1], "\n\n"))
    include_img(
      png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "6.差异网络分析", "Network_CytoscapeStyle.png")
    )
    include_img(
      png_path = file.path(report_path, "6.差异代谢物可视化", group.data[1], "6.差异网络分析", "Network_CytoscapeStyle_name.png")
    )
  ...
  cat("## 组间差异汇总\n\n")
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化/1.venn", "Venn_diagram.png")
  )
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化/1.venn", "Venn_diagram_name.png")
  )
  cat("注：Venn 图：图中不同的圈，表示不同的对比组。\n\n")
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化/2.upset", "UpSet_plot.png")
  )
  include_img(
    png_path = file.path(report_path, "6.差异代谢物可视化/2.upset", "UpSet_plot_name.png")
  )
