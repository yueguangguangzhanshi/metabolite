...
if (file.exists(paste0(report_path, "/3.QC/2.correlation/single/correlation_single.pdf"))) {
  cat('<div style="width: 100%; display: flex; flex-direction: column;">')
  cat(sprintf('![](%s/3.QC/2.correlation/single/correlation_single.pdf){width=100%%, height=500px}', report_path))
...
if (file.exists(paste0(report_path, "/3.QC/3.RSD/single/RSD_single.png"))) {
  cat(sprintf('![](%s/3.QC/3.RSD/single/RSD_single.png)', report_path))
...
if (file.exists(paste0(report_path, "/3.QC/4.PCA/single/PCA_single.png"))) {
  cat(sprintf('![](%s/3.QC/4.PCA/single/PCA_single.png)', report_path))
...
if (final_summary$diff_feature[which(final_summary$ion_type=="single")]%>%sum>0 & final_summary$diff_feature_name[which(final_summary$ion_type=="single")]%>%sum>0){
  ...
  for (group_name in group.data) {
    ...
    cat(paste0("### ", group_name, " \n\n"))
    cat(paste0('* [>打开单离子进行合并的表格](', url_path, '/5.差异筛选/',group_name,'/',group_name,'_表格拆分与去重_single.xlsx)\n\n'))
    cat(" \n\n")
...
对每一组对比，我们对差异代谢物进行聚类，并以热力图进行展示。\n\n")
  ...
  for (group_name in group.data) {
    ...
    cat('<div style="display: flex; justify-content: space-between; gap: 10px; margin-bottom: 20px;">')
    cat('<div style="width: 49%; display: flex; flex-direction: column;">')
    if (file.exists(paste0(report_path,"/6.差异代谢物可视化/",group_name,"/4.VIP_analysis/VIP_distribution.png"))) {
    ...
    cat('</div>')
    cat('<div style="width: 49%; display: flex; flex-direction: column;">')
    if (file.exists(paste0(report_path,"/6.差异代谢物可视化/",group_name,"/4.VIP_analysis/VIP_distribution_metabolite.png"))) {
